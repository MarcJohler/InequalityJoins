...
from classes import IE_self_join, naive_selfjoin_multicond, jvec_ineqjoin_multicond
...
for i in range(test_cases):
    ...
    jvec_join_result = jvec_ineqjoin_multicond(R, R, [j for j in range(predicate_len)], [j for j in range(predicate_len)], operators)
    toc_j = time.perf_counter()
    
    # measure time for naive approach
    naive_join_set = set(naive_join_result)
    #ie_join_set = set(ie_join_result)
    jvec_set = set(jvec_join_result)
    ...
    assert naive_join_set.issubset(jvec_set)
    assert naive_join_set.issuperset(jvec_set)
